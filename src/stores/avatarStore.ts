...
function createAvatarStore() {
  ...
  function saveShouldUpdateAvatarToStorage(userId: number, shouldUpdate: boolean) {
    if (typeof window !== 'undefined') {
      localStorage.setItem(`${STORAGE_KEY_SHOULD_UPDATE_AVATAR_PREFIX}${userId}`, shouldUpdate.toString());
    }
  }
  ...
  function setShouldUpdateAvatarForUser(userId: number, shouldUpdate: boolean): void {
   ...
   function clear(): void {
    ...
    if (typeof window !== 'undefined') {
      ...
      const shouldUpdateKeys = keys.filter(key => key.startsWith(STORAGE_KEY_SHOULD_UPDATE_AVATAR_PREFIX));
