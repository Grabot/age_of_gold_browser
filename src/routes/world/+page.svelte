<script lang="ts">
  ...
  import { accessTokenValue, authStore, shouldUpdateAvatar, userAvatar, userDetail } from '../../stores/authStore';
  import { handleChangeAvatar, handleChangeUsername, handleGetAvatar } from '../../services/settingsService';
  ...
  function handleEditProfileSaveAvatar(data: { avatar?: File | null; defaultAvatar?: boolean | null }) {
    if (data.avatar) {
      const accessToken = $accessTokenValue;
      if (accessToken) {
